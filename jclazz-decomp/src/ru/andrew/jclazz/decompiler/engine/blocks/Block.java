...
public class Block implements CodeItem
{
    ...
    public CodeItem getOperationAfterRecursivly(long start_byte)
    {
        CodeItem res = null;
        for (int i = 0; i < ops.size(); i++)
        {
            ...
            {
                res = ((Block) citem).getOperationAfterRecursivly(start_byte);
                if (res != null) return res;
            }
            else
            {
                if (citem.getStartByte() > start_byte) return citem;
            }
        }
        return null;
    }

    public CodeItem getOperationByStartByteRecursivly(long start_byte)
    {
        CodeItem res = null;
        for (int i = 0; i < ops.size(); i++)
        {
            ...
            if (citem instanceof Block)
            {
                res = ((Block) citem).getOperationByStartByteRecursivly(start_byte);
                if (res != null) return res;
            }
            else
            {
    ...
    public CodeItem getBlockForOperationRecursivly(long start_byte)
    {
        CodeItem res = null;
        for (int i = 0; i < ops.size(); i++)
        {
            ...
            if (citem instanceof Block)
            {
                res = ((Block) citem).getBlockForOperationRecursivly(start_byte);
                if (res != null) return res;
            }
            else
            {
                if (citem.getStartByte() == start_byte) return this;
            }
        }
        return null;
    }
    ...
    public String getOperationsSource(int endByte)
    {
        ...
        for (Iterator i = ops.iterator(); i.hasNext();)
        {
                       ...
                       (citem instanceof Loop && (((Loop) citem).isPrintPrecondition()))) )
            {
                ...
                ln = getMethodView().getMethod().getLineNumberTable().getLineNumber(instruction);
                if (ln != -1)
                {
            ...
            if (citem instanceof Block)
            {
                ...
                {
                    // TODO improve printing line numbers
                    sb.append(indent).append("    ").append("/* ").append(ln).append(" */").append(NL);
                }
            ...
            else
            {
                ...
                if (ov.isPrintable())
                {
                    ...
                    if (opSource != null)
                    {
                        ...
                        sb.append(indent).append("    ").append(opSource).append(";").append(NL);
                    }
                }
            }
        }
