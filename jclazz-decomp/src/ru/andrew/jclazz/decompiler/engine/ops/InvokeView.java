...
//
        ...
        if (isConstructor)
        {
            ...
            if (isAnonymousConstructor)
            {
                ...
                {
                    innerClazz = new Clazz(path + inname + ".class");
                    csv = new AnonymousClazzSourceView(innerClazz, methodView.getClazzView(), methodView.getClazzView().getDecompileParameters());
                    csv.setPrintAsAnonymous(true);
                    //csv.setIndent(anonymousIndent);
                    csv.setIndent("    ");
                    InnerClassView icv = methodView.getClazzView().getInnerClassView(innerClazz.getThisClassInfo().getFullyQualifiedName());
                ...
                {
                    csv = null;
                    items.add("ERROR: Class " + inname + " not found");
                    methodView.getClazzView().getContext().registerError("Class " + inname + " not found");
                }
            }
            // Anonymous Class support v
            else
