...
//        Converter conv = collectTransferInstructions(topBlock);
//
//        // !!!! One iteration
//        new BackLoopDetector().analyze_2(topBlock, conv);
    ...
    // NG if-loop detection
    
    private static Converter collectTransferInstructions(Block block)
    {
        ...
        while (block.hasMoreOperations())
        {
            OperationView view = (OperationView) block.next();
            int code = view.getOpcode();
            Converter.TransferPoint tp = null;
            if ((code >= 153 && code <= 166) || (code >= 198 && code <= 199))
            {
                conv.addTransferPoint(view.getStartByte(), ((IfView) view).getTargetOperation(), true);
            }
            else if (code == 167 || code == 200)
            {
                conv.addTransferPoint(view.getStartByte(), ((GoToView) view).getTargetOperation(), false);
            }
        }
